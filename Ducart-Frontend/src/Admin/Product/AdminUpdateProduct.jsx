...
export default function AdminupdateProduct() {
  ...
  const postData = (e) => {
    ...
    const selectedPics = data.pic instanceof FileList ? Array.from(data.pic) : Array.isArray(data.pic) ? data.pic : [];
  ...
  useEffect(() => {
    if (ProductStateData.length) {
      const item = ProductStateData.find((x) => String(x.id) === String(id));
      if (item) {
                    ...
                    return (
                      <img
                        ...
                        onClick={() => {
                          setData((old) => ({
                            ...old,
                            pic: old.pic.filter((_, pictureIndex) => pictureIndex !== index),
                          }));
                          setFlag(!flag);
                        }}
                        src={`${process.env.REACT_APP_SERVER}${item}`}
                        alt={`Remove ${data.name} picture ${index + 1}`}
                        height={50}
                        width={50}
                        className="mx-1"
                      ></img>
