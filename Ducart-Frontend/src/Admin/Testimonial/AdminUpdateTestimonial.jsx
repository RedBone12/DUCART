...
export default function AdminUpdateTestimonial() {
  ...
  function postData(e) {
    ...
    if (error)
      ...
      let item = TestimonialStateData.find((x) => x.name.toLocaleLowerCase() === data.name.toLocaleLowerCase() && String(x.id) !== String(id))
      if (item) {
        setShow(true)
        setErrorMessage((old) => {
          return {
  ...
  useEffect(() => {
    (() => {
      ...
      if (TestimonialStateData.length)
        setData(TestimonialStateData.find((x) => String(x.id) === String(id)))
    })()
