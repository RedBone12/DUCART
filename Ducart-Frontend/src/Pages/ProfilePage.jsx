...
export default function ProfilePage() {
    ...
    const orders = Array.isArray(CheckoutStateData) ? CheckoutStateData : []
                                            ...
                                            return <tr key={item.id}>
                                                <td>
                                                    <Link to={`${process.env.REACT_APP_SERVER}${item.pic}`} target='_blank' rel='noreferrer'>
                                                        <img src={`${process.env.REACT_APP_SERVER}${item.pic}`} height={50} width={50} className='rounded' alt={item.name} />
                                                    </Link>
                                                </td>
                                                <td>{item.name}</td>
                                                <td>{item.brand}</td>
                                                ...
                                                <td><Link to={`/product/${item.product}`} className='btn btn-primary' aria-label={`View ${item.name}`}><i className='fa fa-shopping-cart'></i></Link></td>
                                                <td><button className='btn btn-danger' onClick={() => deleteRecord(item.id)} aria-label={`Remove ${item.name} from wishlist`}><i className='fa fa-trash'></i></button></td>
                                            </tr>
                                        })
                                    }
                                </tbody>
                                    ...
                                    <div className="table-responsive">
                                        <table className='table table-bordered table-striped table-hover'>
                                            <tbody>
                                                ...
                                                <tr>
                                                    <th>Shipping</th>
                                                    <td>{formatCurrency(item.shipping)}</td>
                                                </tr>
                                                <tr>
                                                    <th>Total</th>
